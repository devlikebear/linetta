...
이미 한 것 ([#62](https://github.com/devlikebear/linetta/pull/62)):

- [x] **세계관 패널** (#28). 에이전트가 MCP로 인물·장소·관계를 만들어도 앱에서 그걸 볼 경로가 씬의 등장 목록과 검색뿐이었다. 종류별 목록·검색·`EntitySheet` 연결을 붙였고, 아웃라인을 새로고침하는 `mcp.changed` 알림이 이제 등장인물 목록도 새로고침한다. 곁들여 `relationships.list` RPC와 `RENDERER_ENGINE_METHODS` 정합성 테스트가 생겼다 — 허용 목록은 `binary_search`로 읽히는데 컴파일러가 지켜 주지 않아서, 빠지거나 순서가 틀리면 기능이 조용히 아무것도 안 한다.
- [x] **등장인물 자동 감지** (#32). 등록된 이름이 본문에 있어도 스캔 버튼을 누르기 전까지 `등장 0`이었다. 이제 타이핑이 멎으면 센다. 세지만 걸지는 않는다 — 동명이인을 엉뚱한 기록에 걸어 버리기 때문에 적용은 작가의 결정으로 남긴다.
- [x] **읽는 사람의 언어로 말하기** (#34, #44, #45). 엔진이 한국어 문장을 써서 영어·일본어 사용자 화면 한가운데 한국어가 나왔다. 엔진은 코드만 내고 앱이 3개 국어로 말한다. 내보내기는 파일이 어디에 떨어졌는지, 그게 동기화 폴더면 어느 폴더인지도 말한다.
