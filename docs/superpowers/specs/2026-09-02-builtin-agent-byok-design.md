...
- **동의가 막는 것은 원고다.** 동의 전에는 씬 본문·요약·등장인물·플롯·팩트 카드 등 작가가 쓴 것이 단 한 글자도 나가지 않는다. `agent.run`도 `providers.test`도 `Source.Client`를 지나며, 그 게이트가 동의 검사 자리다.
- **예외는 모델 목록 조회 하나뿐이다.** `providers.list_models`는 자격 증명이 있으면 동의 전에도 프로바이더에 한 번 접속한다. 설정 화면이 동의 문장을 보여주기 전에 모델 목록을 채워야 하기 때문이다. 나가는 것은 인증 헤더뿐이고 원고는 포함되지 않는다(`engine/internal/provider/catalog.go`). 개인정보 처리방침(#96)은 이 예외를 그대로 적어야 한다.
...
- 추가 2: 프로바이더별 동의. 동의 없이는 **원고가 단 한 글자도** 나가지 않는다 — `providers.test`도 동의 뒤에만. 유일한 예외는 `providers.list_models`로, 자격 증명이 있으면 동의 전에도 모델 목록을 받으러 한 번 접속한다(원고는 나가지 않는다). 5.4 참조.
