...
| **OpenRouter** | One key for many models; optional per-key spend limit; `auto` default model; OAuth PKCE app connect. | Three beginner-friendliness techniques worth copying in spirit: (1) hide provider choice, (2) ship a sane default model, (3) make spend caps visible to kill cost fear. Keep OpenRouter itself as an **advanced** paid option, not the hero. |
