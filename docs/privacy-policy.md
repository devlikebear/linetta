...
- **What goes.** What you type to the agent, the earlier messages in that conversation, and the work and scene you have open. Beyond that, whatever the agent reads through its tools travels to the provider with the request that follows. The tools are not tied to the work you have open: they can list every work in your library with its title, synopsis, and genres, and from any of those works read any scene, the scene and chapter summaries, the outline, full-text search results from across that work's manuscript, the character, plot, and fact cards, and the facts the agent has been told to remember. Those remembered facts live in `<app data>/companion/<project>/memory/experiences.jsonl` rather than in the database, and the agent can add to them as well as read them. The agent is instructed to gather that context before it drafts anything. What the tools can reach is what the consent sentence you tick in Settings describes.
